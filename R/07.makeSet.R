...
#' @description A wrapper function to make.seurat.fdata
...
make.seurat.fdata <- function(seurat, gene.ref.gtf, rda.dir){
  ...
  #  save(gene.gtf, file=file.path(wd, output.dirname,"data/gene.gtf.rda"))
...
#' @title make.seurat.scale.eset
#' @description A wrapper function to make.seurat.scale.eset
#' @usage make.seurat.scale.eset(seurat, fdata, output.dir)
#' @param seurat Seurat object
#' @param fdata feature data
#' @param output.dir output directory
#' @details make expression set using the scaled seurat
#' @return expression set
#' @export
make.seurat.scale.eset <- function(seurat, fdata, output.dir){
  message("[[",Sys.time(),"]] Run to make seurat.scale.eset  --------")
  seurat.scale.eset=make.eset(expr = seurat[['RNA']]@scale.data, pdata = seurat@meta.data, fdata = seurat[['RNA']]@meta.features, verbose = FALSE)
  sum(!rownames(fdata)==featureNames(seurat.scale.eset))
  fData(seurat.scale.eset)=fdata
  save(seurat.scale.eset, file = file.path(output.dir, "seurat.scale.eset.rda"))
  message("[[",Sys.time(),"]] Finish making seurat.scale.eset --------")
  return(seurat.scale.eset)
}
