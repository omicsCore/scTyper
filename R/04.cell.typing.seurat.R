...
#' @description A wrapper function to cell.typing.seurat
#' @usage cell.typing.seurat(seurat, marker="Puram.2017.HNSCC", wd, slot=c("scale.data", "count.data", "data"), cell.typing.method=c("NTP", "ES", "Average"), level=c("cell", "cluster"), assay='RNA', ntp.dir, rda.dir, NTP.g.filter.method=c("sd", "mad", "none"), NTP.gene.filter.cutoff=0.3, NTP.distance=c("cosine","correlation"), NTP.norm.method=c("none", "row.std"), mc.cores=1)
#' @param seurat Seurat object
#' @param marker Which markers to use cell typing
...
#' @param level Whether run cell typing at cluster level
...
cell.typing.seurat=function(seurat,
  ...
  if ((cell.typing.method!="Average") & ("1" %in% as.character(lapply(markerList, function(l) length(l)))) ==TRUE ){
    stop("### Error :: If the number of markers is 1, please use the average method ###")
  }
  ...
  if(cell.typing.method=="NTP"){
    ...
    message("[[",Sys.time(),"]] Run Cell typing using mean expression value--------")
    marker.mean=sapply(markerList, function(a) as.matrix(colMeans(expr[a,], na.rm=T)))
