...
#' @usage run.seurat.process(count.dir = count.dir, rda.dir = rda.dir, project = proj.name, metrics_summary, pheno_info = pheno.df, sample.name = sample.name, min.cells = sum(metrics_summary$Estimated.Number.of.Cells)*0.001, min.features = 200, scale.factor = 10000, vars.to.regress=c("nCount_RNA", "percent.mt"), selection.method = "vst", more_nFeature_RNA = 200, Less_nFeature_RNA = 8000, percent.mt = 10, normalize=TRUE, assay = 'RNA', clust.dims=1:10, resolution=seq(0.6, 2, 0.2), random.seed=1234)
...
#' @usage make.seurat(count.dir, project, min.cells, min.features)
...
make.seurat <- function(count.dir,
  ...
  dim(expression_matrix) # 33694 x 57303
